...
public class ShoesActivity extends Activity implements OnClickListener {
	...
	private NoteDBshoes noteDB;// 创建数据库对象
	...
	public void initView() {
		...
		noteDB = new NoteDBshoes(this);// 实例化数据库对象
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// 通过intent跳转到详情页面
				Intent i = new Intent(ShoesActivity.this, SelectshoesActivity.class);// 直接实例化，两个参数：当前页面，要跳转到的页面
				...
				Log.v("昂", cursor.getString(cursor.getColumnIndex(NoteDBshoes.PATH)));
				Log.v("昂",
						cursor.getString(cursor.getColumnIndex(NoteDBshoes.VIDEO)));
	...
	public void onClick(View v) {
		i = new Intent(this, AddshoesActivity.class);// 实例化i,不管点哪个按钮，都是跳转到AddActivity类
		switch (v.getId()) {
		case R.id.text:
