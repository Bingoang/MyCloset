...
public class SelectskirtActivity extends Activity implements OnClickListener {
	private Button s_delete, s_back;
	private ImageView s_img;
	private VideoView s_video;
	private TextView s_tv;
	private NoteDBskirt noteDB;// 数据库对象
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// System.out.println(getIntent().getIntExtra(NoteDBskirt.ID,
		...
		// 判断有无图片
		if (getIntent().getStringExtra(NoteDBskirt.PATH).equals("null")) {// 如果路径为空，则无图片
			s_img.setVisibility(View.GONE);
		} else {
			s_img.setVisibility(View.VISIBLE);
		}
		// 判断有无视频
		if (getIntent().getStringExtra(NoteDBskirt.VIDEO).equals("null")) {
			s_video.setVisibility(View.GONE);
		} else {
			s_video.setVisibility(View.VISIBLE);
		}
		// 显示文本信息
		s_tv.setText(getIntent().getStringExtra(NoteDBskirt.CONTENT));
		// 显示图片？？？？？？？？？？？估计要改吧
		Bitmap bitmap = BitmapFactory.decodeFile(getIntent().getStringExtra(
				NoteDBskirt.PATH));
		s_img.setImageBitmap(bitmap);
		// 显示视频,并播放
		s_video.setVideoURI(Uri.parse(getIntent().getStringExtra(NoteDBskirt.VIDEO)));
		s_video.start();
	...
	public boolean deleteFile() {
		String fileNameImage=Uri.parse(getIntent().getStringExtra(NoteDBskirt.PATH))+"";
		String fileNameVideo=Uri.parse(getIntent().getStringExtra(NoteDBskirt.VIDEO))+"";
	...
	public void deleteDate() {
		...
		dbWriter.delete(NoteDBskirt.TABLE_NAME,
				"_id=" + getIntent().getIntExtra(NoteDBskirt.ID, 0), null);
	}
